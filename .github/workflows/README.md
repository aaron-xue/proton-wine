...
1. **Environment Setup**
   ...
   - Downloads architecture-specific termuxfs from [WinNative-Emu/proton-wine](https://github.com/WinNative-Emu/proton-wine/releases/tag/Assets)
...
5. **Artifact Packaging**
   ...
   - Generates two `profile.json` files with build metadata:
     - `profile.json` - Proton type for WinNative
     - `profile-wine.json` - Wine type for Winlator for CMOD & Ludashi
   - Creates two output files:
     - **Proton WCP**: Contains Proton-type profile for WinNative (wcp format)
...
- **Termuxfs**: Pre-built termux filesystem from WinNative-Emu/proton-wine (Assets release)
...
```yaml
wget https://github.com/WinNative-Emu/proton-wine/releases/download/<TAG>/termuxfs-${{ matrix.arch }}.tar.gz
```
