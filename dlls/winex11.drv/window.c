...
void get_window_monitors( Display *display, Window window, long *indices )
{
    ...
    if (!XGetWindowProperty( display, window, x11drv_atom(_NET_WM_FULLSCREEN_MONITORS), 0,
                             65536, False, XA_CARDINAL, &type, &format, &count,
...
void detach_client_window( struct x11drv_win_data *data, Window client_window )
{
    ...
    set_wine_allow_flip( client_window, 0 );
...
Window create_client_window( HWND hwnd, RECT client_rect, const XVisualInfo *visual, Colormap colormap )
{
    ...
    {
        set_wine_allow_flip( data->client_window, !!data->whole_window );
...
static void create_whole_window( struct x11drv_win_data *data )
{
    ...
    XChangeProperty( data->display, data->whole_window, x11drv_atom(_NET_WM_PID),
                     XA_CARDINAL, 32, PropModeReplace,
                     (unsigned char *)&pid, 1 );

    UINT_PTR xhwnd = (UINT_PTR)data->hwnd;
    XChangeProperty( data->display, data->whole_window, x11drv_atom(_NET_WM_HWND),
                     XA_CARDINAL, 32, PropModeReplace,
                     (unsigned char *)&xhwnd, sizeof(xhwnd)/4 );
#endif
